...
# MODEL_MAPPING.register(QwenConfig, QwenForSequenceClassification) 
...
task_to_keys = {
    ...
    "sentiment140": ("sentence", None),
    "yelp": ("sentence", None),
    "imdb": ("sentence", None),
    "amazon": ("sentence", None),
}
...
def parse_args():
    ...
    parser.add_argument("--subset_size", 
                        type=int, 
                        default=None, 
                        help="The size of the subset of the dataset to use.")
    
    parser.add_argument("--data_type", 
                        type=str, 
                        choices=["clean", "cfx", "pair_cfx"], 
                        default="clean", 
                        help="Type of data to load: 'clean' for clean dataset, 'cfx' for counterfactual data.")
    
    parser.add_argument(
        "--sym_lambda",
        type=float,
        default=None,
        help="Lambda weight for the symmetry regularization between input and counterfactual losses. Required when data_type is 'pair_cfx'."
    )

    # After all arguments are added and parsed, check condition:
    ...
    parser.add_argument(
        "--measure_energy",
        action="store_true",
        help="Whether to measure energy consumption during training.",
    )
    ##########################
    ...
    if args.subset_size:
        subset_str = f"subset_{args.subset_size}_{args.data_type}"
        args.output_dir = f"{args.output_dir}_{subset_str}"
    ...
    # if args.subset_size in [128, 512]:
    #     args.num_train_epochs = 10
    # elif args.subset_size in [8, 16]:
    #     args.num_train_epochs = 50

    # Force with_tracking to be False even if passed as True
...
def main():
    ...
    if args.filter_disabled and args.mse_alpha == 0:
        dist_method = "kd"
    elif args.filter_disabled and args.mse_alpha > 0:
        dist_method = "lwd"
    elif not args.filter_disabled and args.mse_alpha > 0:
        dist_method = "ted"
    else:
        raise ValueError("Invalid combination of filter_disabled and mse_alpha values.")
    
    if args.data_type == "pair_cfx" and args.sym_lambda is None:
        raise ValueError("--sym_lambda must be provided when data_type is 'pair_cfx'")

    tracker = None
    if args.measure_energy:
        # Create a specific directory for the experiment
        experiment_name = f"{args.task_name}_{args.subset_size}_{dist_method}"
        if args.data_type == "cfx":
            experiment_name += "_cfx"
        energy_log_dir = os.path.join("./energy_logs", experiment_name)
        os.makedirs(energy_log_dir, exist_ok=True)

        # Start the tracker
        tracker = EmissionsTracker(output_dir=energy_log_dir, measure_power_secs=1)
        tracker.start()
    ...
    if args.task_name is not None:
        ...
        #     new_label = ClassLabel(names=["negative", "positive"])

        #     for split in raw_datasets.keys():
        #         raw_datasets[split] = raw_datasets[split].cast_column("label", new_label)
    ...
    if args.task_name is not None:
        ...
        if not is_regression:
            # Use .names to get the list of class names
    ...
    print(f"Student layers: {config.num_hidden_layers}, filter_interval: {args.filter_interval}")
    print(f"Teacher layers: {teacher_config.num_hidden_layers}, teacher_filter_interval: {args.teacher_filter_interval}")
    ...
    def preprocess_function(examples):
        ...
        if "counterfactual" in examples and args.data_type == "pair_cfx":
            cf_result = tokenizer(examples["counterfactual"], padding=padding, max_length=args.max_length, truncation=True)
            result["cf_input_ids"] = cf_result["input_ids"]
            result["cf_attention_mask"] = cf_result["attention_mask"]

        # print("sentence:",result["input_ids"].shape)
        # print("counterfactual:", result["cf_input_ids"].shape)
        # exit(0)
    ...
    with accelerator.main_process_first():
        print("Origninal datasets.")
        print(raw_datasets)

        #TODO: remove this when the dataset is fixed.
        if args.task_name == "sentiment140" or args.task_name == "yelp":
            # Add idx column to validation and test splits if not already present.
            for split in ["validation", "test"]:
                if split in raw_datasets and "idx" not in raw_datasets[split].column_names:
    ...
    if args.with_tracking:
        ...
        exp_name = f"{args.task_name}_{dist_method}_{args.subset_size}_{args.data_type}_seed{args.seed}"


        if args.sym_lambda is not None:
            exp_name += f"_lmb_{args.sym_lambda}"

        experiment_config = vars(args)
        # TensorBoard cannot log Enums, need the raw value
        experiment_config["lr_scheduler_type"] = experiment_config["lr_scheduler_type"].value
        # accelerator.init_trackers("glue_no_trainer", "test_experiemtns", experiment_config)
        accelerator.init_trackers("seed-test", init_kwargs={"wandb": {"name": exp_name }}, config=experiment_config)




    metric = evaluate.load("accuracy")
# Create projection layers if needed for MSE loss

    hidden_state_projections = None
    if args.mse_alpha > 0:
        hidden_state_projections = []
        for _ in range(config.num_hidden_layers // args.filter_interval):
            projection = torch.nn.Linear(config.hidden_size, teacher_config.hidden_size)
            hidden_state_projections.append(projection)
        # Move projections to the correct device
        hidden_state_projections = [proj.to(model.device) for proj in hidden_state_projections]
        # Prepare projections with accelerator
        hidden_state_projections = accelerator.prepare(hidden_state_projections)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            outputs = model(**batch, output_hidden_states=(args.mse_alpha > 0 and args.filter_disabled))
            ...
            mse_loss = 0.0
            if args.mse_alpha > 0:
                student_states = outputs.hidden_states[1:]  # Skip embedding layer
                student_filtered_states = [student_states[(i+1) * args.filter_interval - 1] 
                                        for i in range(config.num_hidden_layers // args.filter_interval)]

                teacher_states = teacher_outputs.hidden_states[1:]
                teacher_filtered_states = [teacher_states[(i+1) * args.teacher_filter_interval - 1] 
                                        for i in range(teacher_config.num_hidden_layers // args.teacher_filter_interval)]

                for idx, (student_state, teacher_state) in enumerate(zip(student_filtered_states, teacher_filtered_states)):
                    # Project student state to match teacher dimensions
                    projected_state = hidden_state_projections[idx](student_state)
                    mse_loss += F.mse_loss(
                        projected_state, teacher_state.detach(), reduction="mean"
                    )
                mse_loss = mse_loss / len(student_filtered_states)

            cfx_loss = 0.0
            if args.data_type == "pair_cfx":
                cf_labels = 1 - batch["labels"]
                cf_outputs = model(
                    input_ids=batch["cf_input_ids"],
                    attention_mask=batch["cf_attention_mask"],
                    labels=cf_labels
                )

                cfx_loss += cf_outputs.loss + args.sym_lambda * torch.abs(outputs.loss - cf_outputs.loss)

            loss = outputs.loss + args.kl_alpha * kl_loss + args.mse_alpha * mse_loss + cfx_loss
        ...
        #     accelerator.log(
        #         {
        #             "accuracy": eval_metric[task_to_metrics[args.task_name]],
            ...
            accelerator.log(
        {
            "accuracy": eval_metric[task_to_metrics[args.task_name]],
            "train_loss": total_loss.item() / len(train_dataloader),
            "kl_loss": kl_loss.item() if args.kl_alpha > 0 else 0,
            "mse_loss": mse_loss.item() if args.mse_alpha > 0 else 0,
            "epoch": epoch,
            "step": completed_steps,
        },
        step=completed_steps,
    ...
    # Stop the energy tracker and save results
    if tracker:
        tracker.stop()
