...
def parse_args():
    ...
    parser.add_argument("--cfx", 
                        action="store_true", 
                        help="Whether or not counterfactuals are included.")
    ...
    if args.subset_size:
        subset_str = f"subset_{args.subset_size}"
        if args.cfx:
            subset_str += "_cfx"
...
def main():
    ...
    if args.task_name is not None:
        ...
        raw_datasets = load_dataset("glue", args.task_name, download_mode="force_redownload")

        print(raw_datasets)
        # raise ValueError("Need to implement the counterfactuals for the GLUE tasks.")

        if args.subset_size and not args.cfx:
            balance = True
            if balance: 
                class_0_indices = [i for i, label in enumerate(raw_datasets["train"]["label"]) if label == 0]
                class_1_indices = [i for i, label in enumerate(raw_datasets["train"]["label"]) if label == 1]
                ...
                # Calculate the number of samples per class for a 50:50 distribution
                num_samples_per_class = args.subset_size // 2
                ...
                # Shuffle the selected indices to mix the classes
                ...
                raw_datasets["train"] = raw_datasets["train"].shuffle(seed=42).select(range(args.subset_size))
                print(raw_datasets)

            print(f"Using subset of size {args.subset_size} for training.")
            # Calculate class imbalance for the subset
            label_counts = Counter(raw_datasets["train"]["label"])
            total_samples = sum(label_counts.values())
            class_imbalance = {label: count / total_samples for label, count in label_counts.items()}

            print("Class Imbalance in the subset:")
            for label, proportion in class_imbalance.items():
        ...
        if args.cfx:
            raw_datasets = load_from_disk(f"./cfx-generator/cfx-dataset/{args.task_name}_{args.subset_size}")
            print(f"Loading cfx dataset from: /cfx-dataset/{args.task_name}_{args.subset_size}")
            # Calculate class imbalance for the subset
            label_counts = Counter(raw_datasets["train"]["label"])
            total_samples = sum(label_counts.values())
            class_imbalance = {label: count / total_samples for label, count in label_counts.items()}
    ...
    if args.with_tracking:
        ...
        exp_name = f"{args.task_name}_{dist_method}_{args.subset_size}"
        if args.cfx:
            exp_name += "_cfx"
        ...
        accelerator.init_trackers("glue_no_trainer", init_kwargs={"wandb": {"name": exp_name }}, config=experiment_config)
